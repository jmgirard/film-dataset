...
holistic_dfw <-  
  ...
  mutate(
    "Positive Affect" = rowMeans(across(c(Alert, Determined, Enthusiastic, Excited, Inspired)), na.rm = TRUE),
    "Negative Affect" = rowMeans(across(c(Afraid, Distressed, Nervous, Scared, Upset)), na.rm = TRUE)
  )
```
...
holistic_dfw |> 
  ggplot(aes(x = `Positive Affect`, y = fct_reorder(Abbrev, .x = `Positive Affect`))) +
  geom_boxplot() +
  labs(y = NULL)
```
...
holistic_dfw |> 
  ggplot(aes(x = `Negative Affect`, y = fct_reorder(Abbrev, .x = `Negative Affect`))) +
  geom_boxplot() +
  labs(y = NULL)
```
...
holistic_dfw2 <- 
  holistic_dfw |> 
  rename_all(.funs = str_replace, pattern = " ", replacement = "")
...
icc_files <- dir(path = "data", pattern = "^icc_.*.csv$", full.names = TRUE)
icc_df <- 
